...
public partial class Painel_Modulos_LegislacaoItem : System.Web.UI.Page
{
    string Param;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (Page.RouteData.Values["Param"] != null)
            {
                ...
                lblIdLegislacaoItemId.Text = dados[1].ToString();
    ...
    protected void CarregarGridItem()
    {
        ...
        DataSet ds = obj.CarregaLegislacaoItem(pesq, Convert.ToInt32(lblLegislacaoId.Text));
    ...
    protected void gvLegislacaoItem_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName.Equals("Editar"))
            {
                ...
                Param = crypt.ActionEncrypt(index + "#" + lblLegislacaoId.Text + "#" + gvLegislacaoItem.Rows[i].Cells[4].Text);

                Response.RedirectToRoute("ModLegislacaoItemEdita", new { Param = Param });
            ...
            if (e.CommandName.Equals("Excluir"))
            {
                ...
                legis = obj.GetLegislacaoItemById(Convert.ToInt32(lblIdLegislacaoItemId.Text));

                obj.AtualizaLegislacaoItem(legis, 3);

                string url = HttpContext.Current.Request.Url.AbsoluteUri;
                FuncoesBLL objf = new FuncoesBLL();
                objf.Atividades(url, "Excluiu Legislação Item", ControleAcesso.LerTicket().CadastroId);
            ...
            if (e.CommandName.Equals("Subir"))
            {
                ...
                if (Ordem == 1)
                {
                    ...
                    lblMsgTit.Text = "Legislação é o primeiro da Ordem";
                ...
                {
                    obj.OrdernaLegislacaoItem(Convert.ToInt32(e.CommandArgument), Ordem, 1);
                    CarregarGridItem();
                }
            }
            ...
            {
                int Ordem = Convert.ToInt32(gvLegislacaoItem.Rows[i].Cells[4].Text);
                int OrdemMax = obj.GetLegislacaoItemOrdem(Convert.ToInt32(lblLegislacaoId.Text));
                if (Ordem == OrdemMax)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "openModal();", true);
                    Label lblMsgTit = Page.Master.FindControl("lblTitulo") as Label;
                    lblMsgTit.Visible = true;
                    lblMsgTit.Text = "Legislação é o último da Ordem";
                ...
                {
                    obj.OrdernaLegislacaoItem(Convert.ToInt32(e.CommandArgument), Ordem, 2);
                    CarregarGridItem();
                }
            }
        }
    ...
    {
        Param = crypt.ActionEncrypt("0" + "#" + lblLegislacaoId.Text + "#" + "0");
        Response.RedirectToRoute("ModLegislacaoItemEdita", new {Param = Param });
    }
    ...
    {
        ModuloLegislacaoItemBLL obj = new ModuloLegislacaoItemBLL();
        DataSet ds = obj.CarregaLegislacaoItem(txtPesquisa.Text.Trim(), Convert.ToInt32(lblLegislacaoId.Text));
        gvLegislacaoItem.DataSource = ds;
        gvLegislacaoItem.DataBind();
    }
}