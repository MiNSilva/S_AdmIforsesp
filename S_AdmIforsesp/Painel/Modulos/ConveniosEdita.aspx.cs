...
public partial class Painel_Modulos_UtilidadesEdita : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Page.RouteData.Values["Param"] != null)
            {
                ...
                {
                    ModuloConvenioBLL obj = new ModuloConvenioBLL();
                    ModuloConvenioModel uti = new ModuloConvenioModel();

                    uti.ConvenioId = Convert.ToInt32(lblUtilidadeId.Text);

                    DataSet ds = obj.CarregaConveniosId(uti, 1);
    ...
    protected void btnSalvar_Click(object sender, EventArgs e)
    {
        try
        {
            ModuloConvenioModel util = new ModuloConvenioModel();
            ModuloConvenioBLL obj = new ModuloConvenioBLL();
            ...
            if (lblUtilidadeId.Text == "0")
            {
                ...
                erro = VerificaTamanho(FileUpload1, Largura, Altura);
            ...
            else
            {
                ...
                util.ConvenioId = Convert.ToInt32(lblUtilidadeId.Text);
                util.Ordem = 0;

            }

            // Apagando a imagem antiga
            //string caminho = Server.MapPath("~/images/Convenios/" + txtImgOriginal.Text.Trim());
            ...
            util.Descricao = txtDescricao.Text;

            string path = Server.MapPath("~/images/Convenios/" + util.Imagem);
            if (!File.Exists(path))
            {
            ...
            objf.Atividades(url, "Inseriu ou Alterou Convenio", ControleAcesso.LerTicket().CadastroId);

            Response.RedirectToRoute("ModConvenios", new object { });
        }
        catch (Exception ex)
        {
