    ...
    protected void gvUtilidade_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName.Equals("Editar"))
            {
                ...
                Response.RedirectToRoute("ModConveniosEdita", new { Param = Param });
            ...
            if (e.CommandName.Equals("Excluir"))
            {
                ...
                util.Ordem = 0;
                util.ConvenioId = Convert.ToInt32(lblIdUtilidade.Text);
                util.Externo = 0;
                util.Descricao = string.Empty;

                obj.Atualiza(util, 3);

                string url = HttpContext.Current.Request.Url.AbsoluteUri;
                FuncoesBLL objf = new FuncoesBLL();
                objf.Atividades(url, "Excluiu Convenio", ControleAcesso.LerTicket().CadastroId);

                string caminho = Server.MapPath("~/images/Convenios/" + util.Imagem);
            ...
            {
                int Ordem = Convert.ToInt32(gvUtilidade.Rows[i].Cells[5].Text);
                if (Ordem == 1)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "openModal();", true);
                    Label lblMsgTit = Page.Master.FindControl("lblTitulo") as Label;
                    lblMsgTit.Visible = true;
                    lblMsgTit.Text = "Convenio é a primeira da Ordem";
            ...
            if (e.CommandName.Equals("Descer"))
            {
                ...
                if (Ordem == OrdemMax)
                {
                    ...
                    Label lblMsgTit = Page.Master.FindControl("lblTitulo") as Label;
                    lblMsgTit.Visible = true;
                    lblMsgTit.Text = "Convenio é a última da Ordem";
